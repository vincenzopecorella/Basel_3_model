...
from constants import DATA_FOLDER, SOURCES_FILE_NAME_OPTIMIZED
...
for article in articles_formatted_no_duplicates:
    change1 = article.article_content.replace("\xa0\n\n\n", " ")
    change2 = change1.replace("\n\n\n\n\n", "\n")
    change3 = change2.replace("\n\n\n", "\n")
    change4 = change3.replace("\xa0\xa0\n\n"," ")
    articles_formatted_no_duplicates_cleaned.append(Article(article_title=article.article_title,
                                          article_number=article.article_number,
                                          article_content=change4))

save_path = str(PROJECT_DIR) + f"/{DATA_FOLDER}{SOURCES_FILE_NAME_OPTIMIZED}"
open_file = open(save_path, "wb")
pickle.dump(articles_formatted_no_duplicates_cleaned, open_file)
open_file.close()
